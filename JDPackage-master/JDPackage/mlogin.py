...
def get_dat(username, password, rand):
    dat = """
    <script>
    ...
    function getDat(username,pwd)""" + str(rand) + """document.write(getDat('""" + str(username) + """','""" + str(
        password) + """'));</script> """
    open('dat.html', 'w').write(dat)
...
document.write(u());
 ...
 '''
    open('pwd.html', 'w').write(a)


def login(username, pwd, rk_um, rk_pwd):
    url = 'http://passport.m.jd.com/user/login.action?returnurl=https://m.jd.com?indexloc=1'
    cookiestr = ''
    ...
    get_dat(username, pwd, rand)
    get_pwd(pwd, str_rsaString)
    driver = webdriver.PhantomJS(executable_path=r"C:\Python27\phantomjs.exe")  # li 2018.09
    driver.get('file:///' + os.getcwd().replace("\\", "/") + '/dat.html')
    dat = re.findall('<body>(.*?) </body>', driver.page_source, re.S)[0]
    driver.get('file:///' + os.getcwd().replace("\\", "/") + '/pwd.html')
