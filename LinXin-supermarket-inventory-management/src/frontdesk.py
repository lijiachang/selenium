...
class FrontDesk:
    ...
    def meta(self):
        ...
        while (True):
            ...
            if cmd == "1":
                ...
                self.query_my_all_sell(self.cashier_id)
    ...
    def return_good(self):
        """退货功能"""
        com_num = input("请输入交易流水号:").strip()
        sell_info = Basic.queryOneSellFlowNum(com_num)
        if not sell_info:
            print("交易流水号不存在.")
            return
        good_num = sell_info[3]  # 退货数量加到库存
        good_no = sell_info[1]  # 商品编号
        # 1. 增加库存数量
        try:
            Basic.addOneCommodityCnt(good_no, good_num)
            print("操作成功.")
        except Exception as e:
            print("操作失败，原因：", e)
        # 2. 删除销售记录
        Basic.del_one_sell(com_num)

        print('退货成功！商品{}库存已增加'.format(good_no))

    def query_my_all_sell(self, id):
        info = Basic.queryMySell(id)
