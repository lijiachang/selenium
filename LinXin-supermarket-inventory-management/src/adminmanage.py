...
class AdminManage:
    ...
    def metaQuery(self):
        while True:
            ...
            print("4: 查看所有售货信息")
            print("5: 查看所有进货信息")
            # print("6: 查看单个销售员信息(未开发)")
            # print("7: 查看单个采购员信息(未开发)")
    ...
    def addOneCashier(self):
        '''前台'''
        cash_no = input("请输入增加销售员的编号:").strip()
        ...
        cash_hourse = input("请输入该员工的地址:").strip()
        cash_salary = float(input("请输入该员工的月工资:").strip())
        cash_phone = input("请输入该员工的手机号:").strip()
        cash_edu = input("请输入该员工的教育背景(1-5):").strip()
        try:
            Basic.addOneCashier(cash_no, cash_name, cash_pwd, cash_sex, cash_age, cash_hourse, cash_salary, cash_phone,
                                cash_edu)
            print("添加销售员成功.")
    ...
    def addOnePurchaser(self):
        ...
        pur_entry = input("请输入该员工的入职日期(eg: 2019-6-4):").strip()
        pur_edu = input("请输入该员工的教育背景(1-5):").strip()
        try:
            Basic.addOnePurchaser(pur_no, pur_name, pur_pwd, pur_sex, pur_age, pur_salary, pur_phone, pur_entry,
                                  pur_edu)
            print("添加采购员成功.")
        except Exception as e:
            print("添加采购员失败,原因:", e)
    ...
    def delOneCashier(self):
        ''''''
        cash_no = input("请输入要移除的销售员的编号:").strip()
        cash = Basic.queryOneCashier(cash_no)
        if cash == []:
            print("不存在该员工.")
            return
    ...
    def modifyOneCashier(self):
        cash_no = input("请输入需要修改的销售员的编号:").strip()
        cash = Basic.queryOneCashier(cash_no)
        if cash == []:
            print("该销售员不存在.")
            return
        ...
        cash_sex = input("请输入该员工的性别:").strip()
        cash_age = int(input("请输入该员工的年龄:").strip())
        cash_hourse = input("请输入该员工地址:").strip()
        cash_salary = float(input("请输入该员工的月工资:").strip())
        cash_phone = input("请输入该员工的手机号:").strip()
        ...
        try:
            Basic.modifyOneCashier(cash_no, cash_name, cash_pwd, cash_sex, cash_age, cash_hourse, cash_salary,
                                   cash_phone, cash_entry, cash_edu)
            print("修改成功.")
        except Exception as e:
            print("修改失败,原因:", e)

    def modifyOnePurchaser(self):
        pur_no = input("请输入需要修改的采购员的编号:").strip()
        pur = Basic.queryOnePurchase(pur_no)
        if pur == []:
            print("该销售员不存在.")
            return
        ...
        pur_edu = input("请输入该员工的教育背景(1-5):").strip()
        try:
            Basic.modifyOnePurchaser(pur_no, pur_name, pur_sex, pur_age, pur_salary, pur_phone, pur_entry, pur_edu)
            print("修改成功.")
        except Exception as e:
            print("修改失败,原因:", e)
    ...
    def modifyOneCommodity(self):
        ...
        try:
            Basic.modifyOneCommodity(com_num, com_name, com_type, com_size, com_price, com_mdate, com_edate,
                                     com_quantity)
            print("修改商品成功")
        except Exception as e:
            print("修改商品失败,原因：", e)
