...
if __name__ == "__main__":
    sqlComm = """select '有' || count(*) || '个节点inactive redo log少于2个(' || listagg(thread#, ',') within group(order by thread#) || ')' TITLE, count(*) COUNTS from (select thread#, count(*) cnt from v$log where status = 'INACTIVE' group by thread# having count(*) < 2);"""
    ...
    results = list()
    results.append(separator.join([mpoint, ckbp, "PM-Ora-Redolog-Counts", str(result[0].get("COUNTS"))]))
