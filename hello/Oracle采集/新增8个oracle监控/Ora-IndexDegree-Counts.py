...
if __name__ == "__main__":
    sqlComm = """select '有' || count(*) || '个索引degree为default,'||listagg('索引名:'|| owner || '.' || index_name || ',') within group(order by owner) title, count(*) count from (select t.owner, t.index_name, t.table_owner, t.table_name, t.degree from dba_indexes t where t.degree = 'DEFAULT' and t.owner in (select a.username from dba_users a where a.profile in ('PROF_WH', 'PROF_PROD')))"""
    ...
    results = list()
    results.append(separator.join([mpoint, ckbp, "PM-Ora-IndexDegree-Counts", str(result[0].get("COUNT"))]))
