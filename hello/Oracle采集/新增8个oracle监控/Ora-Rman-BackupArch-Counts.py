...
if __name__ == "__main__":
    sqlComm = """select listagg('归档报错的库名有：'||db_name|| ',') within group(order by ip) TITLE, count(1) COUNTS from (select db_name, ip, max(archive_log) last_backuparch from system.ENMO_CHECKBACKUP_LIST t group by db_name, ip) where last_backuparch is null;"""
    ...
    results = list()
    results.append(separator.join([mpoint, ckbp, "PM-Ora-Rman-BackupArch-Counts", str(result[0].get("COUNTS"))]))
