...
if __name__ == "__main__":
    sqlComm = """select '有' || count(*) || '个实例启动时间小于15分钟,分别是：'||listagg(inst_id, ',') within group(order by inst_id) TITLE， count(*) COUNTS from (select inst_id, startup_time from gv$instance where startup_time > sysdate - 15 / 60 / 24);"""
    ...
    results = list()
    results.append(separator.join([mpoint, ckbp, "PM-Ora-Startup-Counts", str(result[0].get("COUNTS"))]))
