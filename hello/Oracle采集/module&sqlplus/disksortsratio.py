...
if __name__ == "__main__":
    sqlComm = "SELECT disk.Value disk,mem.Value mem,(disk.Value/mem.Value)*100 ratio                   FROM v\$sysstat disk,v\$sysstat mem                       WHERE mem.NAME='sorts (memory)' AND disk.NAME='sorts (disk)';"
    if model == 'sqlplus':
    ...
    DiskSorts = str(result[0].get("RATIO"))
    DiskSorts = "0" + DiskSorts if DiskSorts[0] == "." else DiskSorts
    ...
    results = list()
    results.append(
        separator.join([mpoint, ckbp, "APM-00-03-01-00-04", str(DiskSorts)]))  # DiskSorts磁盘排序百分比(磁盘排序比率)
    for r in results:
        print(r)
