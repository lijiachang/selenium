...
if __name__ == "__main__":
 ...
 where tablespace_name =
       (select value from v\$parameter where name = 'undo_tablespace') and status='EXPIRED'
 group by status;"""  # ����$ ��Ҫת��
    if model == 'sqlplus':
    ...
    results = list()
    results.append(separator.join([mpoint, ckbp, "FM-ORACLE-AVAIABILITY-UndoExpired", str(result[0].get("SIZE_M"))]))
