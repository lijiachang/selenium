...
def QueryBySqlPlus(sqlCommand):
    ...
    if not list_1:
        ...
        print("%s||||PM-ORACLE-PERFORMANCE-LogSwitches||Null is OK" % mpoint)  # SQL�޷�������������
        sys.exit(0)
...
if __name__ == "__main__":
    sqlComm = """select round((1 - (physical.value - direct.value - lobs.value) / logical.value) * 100, 2) "Buffer Cache Hit Ratio"
 ...
 where physical.name = 'physical reads'
   ...
   and logical.name = 'session logical reads';"""  # ����$ ��Ҫת��
    if model == 'sqlplus':
    ...
    results = list()
    results.append(separator.join([mpoint, ckbp, "PM-ORACLE-PERFORMANCE-BuffHitRatio", str(result[0].get("Buffer Cache Hit Ratio"))]))
