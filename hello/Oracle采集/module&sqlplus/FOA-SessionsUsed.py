...
if __name__ == "__main__":
    sqlComm = "SELECT ROUND(current_utilization / LIMIT * 100) || '%' rate   FROM (SELECT resource_name,                current_utilization,                max_utilization,                TO_NUMBER(initial_allocation) LIMIT           FROM v$resource_limit          WHERE resource_name IN ('sessions')            AND max_utilization > 0);"
    if model == 'sqlplus':
    ...
    results = list()
    SessionsUsed = str(result[0].get("RATE"))
    SessionsUsed = str(result[0].get("RATE"))[:-1] if SessionsUsed[-1] == "%" else SessionsUsed
    results.append(separator.join([mpoint, ckbp, "FM-ORACLE-AVAIABILITY-SessionsUsed", SessionsUsed]))
