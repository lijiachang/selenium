...
if __name__ == "__main__":
    ...
    sqlComm2 = "select sum(s.value)  read  from v\$sysstat s where s.name='session logical reads';"  # 符号$需要转义

    if model == 'sqlplus':
        wait = QueryBySqlPlus(sqlComm)
        read = QueryBySqlPlus(sqlComm2)
    elif model == 'module':
        sqlComm = sqlComm.replace("v\$", "v$")
        sqlComm2 = sqlComm2.replace("v\$", "v$")
        wait = QueryByModule(sqlComm)
        read = QueryByModule(sqlComm2)
    else:
        read, wait = 0, 0
