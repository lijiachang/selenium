...
if __name__ == "__main__":
    sqlComm = """select value from v\$parameter where name ='processes';"""  # 符号$ 需要转义,最后加;号
    if model == 'sqlplus':
    ...
    results = list()
    results.append(separator.join([mpoint, ckbp, "CM-MaxProcess", str(result[0].get("VALUE"))]))  # 数据库允许的最大连接数
