...
By：李家昌  最后修改：2018.05.15
表格增加 操作系统  列
"""
reload(sys)
sys.setdefaultencoding("utf-8")
#######################################################################
data = load_workbook(u"UltraAgent脚本信息集oraclemysql.xlsx")  # 读取excel表
sheet = data[u"MySQL"]                              # 读取表名
file_sql = "MySQL_0612_for_oracle.sql"         # 生成的sql文件
#######################################################################
...
def update_data(row_index, is_super=1):
    ...
    e = sheet[row_index]
    kpi = e[15].value   # 每次循环都更新 kpi内数据 3.19 （解决合并单元格，第二行重复添加 kpi）
    if is_super == 1:
        if e[0].value is not None:
            mpointClass = e[0].value.strip()  # 监控类型
        scriptName = e[3].value.strip()  # 脚本名
        scriptNote = e[4].value.strip()  # 脚本说明
    else:
        print("is_not_super")

    adaptos = adaptos if e[1].value is None else str(e[1].value).strip()   # 操作系统
    scriptPath = e[2].value.replace("\\", "/").strip() if e[2].value is not None else scriptPath  # 脚本路径
    creator = e[5].value.strip()  # 所属用户
    parameterName = "" if e[6].value is None else str(e[6].value).strip()  # 参数名称
    parameterDescr = "" if e[7].value is None else str(e[7].value).strip()  # 参数描述
    parameterDataType = "" if e[8].value is None else str(e[8].value).strip()  # 参数储存类型
    parameterDataScope = "" if e[9].value is None else str(e[9].value).strip()  # 参数范围
    isRequired = "1" if str(e[10].value).strip() == u"是" else "0"  # 是否必填
    isEncrypted = "1" if str(e[11].value).strip() == u"是" else "0"  # 是否加密
    defaultValue = "" if e[12].value is None else str(e[12].value).strip()  # 默认值
    execsEquence = "" if e[13].value is None else str(e[13].value).strip()  # 执行顺序  直接取到的是float...
    note = "" if e[14].value is None else str(e[14].value).strip()  # 参数说明
