...
# 地址
PAGE10_1_1 = r"https://pb.jd.com/activity/2018/bk/html/index.html?utm_user=plusmember&cu=true,true&utm_source=kong&utm_medium=tuiguang&utm_campaign=t_335140509_&utm_term=70b8976073024140a4008d5906a92656&entranceId=Hv4flQvbzDeUSC0w"
...
root.geometry('550x400+600+400')  # 设置默认窗口的大小宽x高+偏移量
# root.resizable(False, False)  # 固定窗口大小
root.title("广发 京东100-50 10点自动抢 作者QQ944581577")
...
# text = tk.Text(frame_right_bottom, height=15, width=30, bg="#7A7A7A")  # 日志输出
...
scrolW = 32  # 设置文本框的长度
scrolH = 15  # 设置文本框的高度
text = ScrolledText.ScrolledText(frame_right_bottom, width=scrolW, height=scrolH, wrap=tk.CHAR)
...
# text.grid(column=0, columnspan=3)
# #  键盘输入不会被插入到文本框:
text.bind("<KeyPress>", lambda e: "break")
text.pack()
...
def test():
    for one_cookie in cookie_list_dict:
        data = time.strftime('%d日%H:%M ', time.localtime(time.time()))
        result_PAGE10_1_1 = update_data(PAGE10_1_1, one_cookie)
        print(result_PAGE10_1_1)
        #result_PAGE10_1_2 = update_data(PAGE10_1_2, one_cookie)
        #print(result_PAGE10_1_2)
        # result_PAGE10_1_3 = update_data(PAGE10_1_3, one_cookie)
        # print(result_PAGE10_1_3)

        text.insert("end", data + result_PAGE10_1_1 + "\n")

        # if u"京东闪付满10元减1元" in result_PAGE10_1_2:
        #     text.insert("end", "{0} 2闪付券10-1，领取成功\n".format(data))
        # elif u"今日已领取" in result_PAGE10_1_2:
        #     text.insert("end", "{0} 2闪付券10-1，已领取过\n".format(data))
        # else:
        #     text.insert("end", data + result_PAGE10_1_2 + "\n")

        # if u"优惠券领取成功" in result_PAGE10_1_3:
        #     text.insert("end", "{0} 3付款码10-1，领取成功\n".format(data))
        # elif u"今日已领取" in result_PAGE10_1_3 or "MKT9000021" in result_PAGE10_1_3:
        #     text.insert("end", "{0} 3付款码10-1，已领取过\n".format(data))
        # else:
        #     text.insert("end", data + result_PAGE10_1_3 + "\n")
    text.see(tk.END)  # 一直显示最新的一行
    text.update()


button2 = tk.Button(root, text="检查所有cookie", bg="#9ACD32", command=test_click)
button2.pack(side=tk.LEFT)

button_test = tk.Button(root, text="立即领取", bg="red", command=test)  # 测试按钮 test
...
def update_data(PAGE, cookie):
    ...
    headers = {'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8',
               ...
               'Cookie': "shshshfpa=b7e0f1d0-fe95-d06d-000b-eb85b6f2b46a-1543370893; shshshfpb=07c02a9e764134490a779014ae41cdd4e922fa604a47631295bfdf890c; pinId=g-aU7U9xYcr1BtJPsvi3YbV9-x-f3wj7; pin=jd_76ab57cdbf964; _tp=ROmvWIF2YFJGt8lj8XBFsArJ01QMV7d6rocoJiJXxAM%3D; _pst=jd_76ab57cdbf964; __jdu=1463683099; areaId=13; ipLoc-djd=13-1022-28932-51342; unick=%E5%B7%B4%E5%95%A6%E5%95%A6%E5%B0%8F%E6%91%A9%E4%BB%99; mt_xid=V2_52007VwMXVV9fW1geQRFsAGUAQlUKDFFGTxoQXxliBkJUQVBQD01VGVUHYQVBVl8NVV0deRpdBW4fElFBWFdLHkESXgZsABZiX2hRahtKH1wAYDMSVlw%3D; user-key=4c4f5036-0e0b-4f29-ba5c-86808507ed8b; cn=0; unpl=V2_ZzNtbRZUQ0d3AU5deh0OBGIAFA4RVEcQcABFA3gRCwcwC0UKclRCFXwURldnGVoUZwMZX0JcRxZFCEdkfxtfDGACEFVyZ0Ildgh2VHopXABgARVUQFZLHUU4QWRLQAxbZAQbbUBQRRNzDERReSldNWUFE19AVUMXdTgNOnpUXABgARVUQFZLHUUJdlc%3d; PCSYCityID=1; TrackID=1Wz58ssgbkkL3uaJTGHLk5zQ_DOiEaIglWnjTCIEBrh8HfsdEvZhKrnntkRJ5nXULJBFI36QqGwfsQxZNcNQAR8C3BF8tqENmQttcD8xijSc; __jda=226489691.1463683099.1542880914.1545273788.1545360388.22; __jdc=226489691; _jrda=1; shshshfp=835048ebbb6e4a91ccaa0e6ad690749a; shshshsID=80420a7cee8fc0960511a9ae72988ad4_1_1545360388567; mba_muid=1463683099; mba_sid=15453603886114500184943812550.1; TrackerID=hO3bhSLPEC8utkHbh4YSEFP1KgazfTIYZww80rB_nBpCHBKSdCnXxwlnz0eU8MkWg6uxtuYHhsLHXmWI60PltYXo3rVxj5fKaCqxqRDZJxA; pt_key=AAFcHFQLADB7N5LnfMpmuNnddSiBFUaeJOIIovggCXgMeWqe7lQn7RTYM4gn8YVBh8OMTTuSI40; pt_pin=jd_76ab57cdbf964; pt_token=j8ybwpwu; pwdt_id=jd_76ab57cdbf964; __jdb=226489691.4.1463683099|22.1545360388; __jdv=226489691|kong|t_335140509_|tuiguang|70b8976073024140a4008d5906a92656|1545360398292; _jrdb=1545360398328",
               'Host': 'pb.jd.com',
               'Referer': 'https://plogin.m.jd.com/user/login.action?appid=100&returnurl=https%3A%2F%2Fpb.jd.com%2Factivity%2F2018%2Fbk%2Fhtml%2Findex.html%3Futm_user%3Dplusmember%26cu%3Dtrue%2Ctrue%26utm_source%3Dkong%26utm_medium%3Dtuiguang%26utm_campaign%3Dt_335140509_%26utm_term%3D70b8976073024140a4008d5906a92656%26entranceId%3DHv4flQvbzDeUSC0w',
    ...
    if req.encoding == 'ISO-8859-1':
        encodings = utils.get_encodings_from_content(req.text)
        if encodings:
...
def update_page():
    ...
    print(now2)
    if now == "10.00":
        test()  # 领取10-1 不用抢，不必多线程
        # threads = []
        # for one_cookie in cookie_list_dict:
        #     t = threading.Thread(target=judge_result, args=(PAGE10_1_1, one_cookie,))  # 注意args后面有（）里面还有逗号,
        #     threads.append(t)
