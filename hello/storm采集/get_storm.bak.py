...
ip = "192.168.95.111"
# Storm 服务监控
cluster_configuration = "".join(["http://", ip, ":58087/api/v1/cluster/configuration"])
cluster_summary = "".join(["http://", ip, ":58087/api/v1/cluster/summary"])
# storm控制节点监控
nimbus_summary = "".join(["http://", ip, ":58087/api/v1/nimbus/summary"])
# storm工作节点监控
supervisor_summary = "".join(["http://", ip, ":58087/api/v1/supervisor/summary"])
# storm拓扑监控
topology_summary = "".join(["http://", ip, ":58087/api/v1/topology/summary"])
...
results = "".join(["192.168.95.111-58087-storm||ResStormService||CM-storm-01-01||" +
                   "serviceName"])  # 未知
results = "\n".join([results, "192.168.95.111-58087-storm||ResStormService||CM-storm-01-02||" +
                     "descr"])  # 未知
results = "\n".join([results, "192.168.95.111-58087-storm||ResStormService||CM-storm-01-03||" +
                     dict_cluster_summary['stormVersion']])
results = "\n".join([results, "192.168.95.111-58087-storm||ResStormService||CM-storm-01-04||" +
                     get_zookeeperAddr()])  # 1.0:未知  2.0:用函数计算
results = "\n".join([results, "192.168.95.111-58087-storm||ResStormService||CM-storm-01-05||" +
                     str(len(dict_cluster_configuration["nimbus.seeds"]))])  # 1.0：计算nimbus.seeds的数量 来推算nimbusNum 2.0：同
results = "\n".join([results, "192.168.95.111-58087-storm||ResStormService||CM-storm-01-06||" +
                     str(dict_cluster_summary['supervisors'])])
results = "\n".join([results, "192.168.95.111-58087-storm||ResStormService||PM-storm-01-01||" +
                     "0"])  # 1.0:未知   2.0:能采集到上面的数据，说明正常 0
results = "\n".join([results, "192.168.95.111-58087-storm||ResStormService||PM-storm-01-02||" +
                     str(dict_cluster_summary['slotsUsed'])])
results = "\n".join([results, "192.168.95.111-58087-storm||ResStormService||PM-storm-01-03||" +
                     str(dict_cluster_summary['slotsFree'])])
results = "\n".join([results, "192.168.95.111-58087-storm||ResStormService||PM-storm-01-04||" +
                     str(dict_cluster_summary['slotsTotal'])])
results = "\n".join([results, "192.168.95.111-58087-storm||ResStormService||PM-storm-01-05||" +
                     str(dict_cluster_summary['executorsTotal'])])
results = "\n".join([results, "192.168.95.111-58087-storm||ResStormService||PM-storm-01-06||" +
...
while n < len(dict_nimbus_summary['nimbuses']):
    results = "\n".join([results, "192.168.95.111-58087-storm||ResStormNimbus||CM-storm-02-01||" +
                         dict_nimbus_summary['nimbuses'][n]['host']])
    results = "\n".join([results, "192.168.95.111-58087-storm||ResStormNimbus||CM-storm-02-02||" +
                         str(dict_nimbus_summary['nimbuses'][n]['port'])])
    results = "\n".join([results, "192.168.95.111-58087-storm||ResStormNimbus||CM-storm-02-03||" +
    ...
    results = "\n".join([results, "192.168.95.111-58087-storm||ResStormNimbus||PM-storm-02-01||" +
...
results = "\n".join([results, "192.168.95.111-58087-storm||ResStormSupervisor||CM-storm-03-01||" +
                     dict_supervisor_summary['supervisors'][0]['host']])
results = "\n".join([results, "192.168.95.111-58087-storm||ResStormSupervisor||CM-storm-03-02||" +
                     dict_supervisor_summary['supervisors'][0]['id']])
results = "\n".join([results, "192.168.95.111-58087-storm||ResStormSupervisor||PM-storm-03-01||" +
                     "0"])  # 1.0 status属性在json里面没有找到，后续处理  2.0 参考java代码，写死"0"
results = "\n".join([results, "192.168.95.111-58087-storm||ResStormSupervisor||PM-storm-03-02||" +
                     str(dict_supervisor_summary['supervisors'][0]['slotsUsed'])])
results = "\n".join([results, "192.168.95.111-58087-storm||ResStormSupervisor||PM-storm-03-03||" +
                     str(dict_supervisor_summary['supervisors'][0]['slotsTotal'])])
results = "\n".join([results, "192.168.95.111-58087-storm||ResStormSupervisor||PM-storm-03-04||" +
...
n = 1
while n <= numbers:
    results = "\n".join([results, "192.168.95.111-58087-storm||ResStormTopology||CM-storm-04-01||" +
                         dict_topology_summary['topologies'][n - 1]['name']])
    results = "\n".join([results, "192.168.95.111-58087-storm||ResStormTopology||CM-storm-04-02||" +
                         dict_topology_summary['topologies'][n - 1]['id']])
    results = "\n".join([results, "192.168.95.111-58087-storm||ResStormTopology||PM-storm-04-01||" +
                         dict_topology_summary['topologies'][n - 1]['status']])
    results = "\n".join([results, "192.168.95.111-58087-storm||ResStormTopology||PM-storm-04-02||" +
                         str(dict_topology_summary['topologies'][n - 1]['workersTotal'])])
    results = "\n".join([results, "192.168.95.111-58087-storm||ResStormTopology||PM-storm-04-03||" +
                         str(dict_topology_summary['topologies'][n - 1]['executorsTotal'])])
    results = "\n".join([results, "192.168.95.111-58087-storm||ResStormTopology||PM-storm-04-04||" +
                         str(dict_topology_summary['topologies'][n - 1]['tasksTotal'])])
    results = "\n".join([results, "192.168.95.111-58087-storm||ResStormTopology||PM-storm-04-05||" +
