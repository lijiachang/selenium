...
################# 配置区 ##############################################################################################
domain = 'www.lawcdyc.com'  # 网站地址 www.xxx.com
domain_index = 'http://www.lawcdyc.com/admin759.php'  # 管理后台的页面URL
username = 'admin129czz'  # 后台管理用户名
password = 'o5EmgdcIiX6j914r'  # 后台管理密码

# Excel配置
excel_name = '/root/auto/lawcdyc200m.xlsm'
sheet_name = 'Sheet1'
# 替换的关键字
...
User-Agent: Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.198 Safari/537.36"""
        form = {'username': username,
                'password': password}
        url = 'http://www.lawcdyc.com/index.php?m=site&c=index&a=login'
...
class ArticleForm:
    ...
    def init_publish_page(self):
        """1.获取发布页面的access_key值，虽然自己随机一个也能用，但是防止撞库，还是从页面获取值比较好
           2.获取发布页的 城市和id 映射关系"""
        url = 'http://www.lawcdyc.com/index.php?m=site&c=content&a=article_form'
    ...
    def post_tag(self, tag_str):
        ...
        请求 URL: http://www.lawcdyc.com/index.php?m=mod&c=tag_article&a=ajax_form&tag_str=n%E9%B9%A4%E5%B2%97,50,%E5%AD%90,%E6%80%8E%E4%B9%88%E8%A3%85%E4%BF%AE,%E8%AE%BE%E8%AE%A1%E5%9B%BE
        表单：
        m: mod
        c: tag_article
        a: ajax_form
        ...
        form = {'m': 'mod',
                ...
                'a': 'ajax_form',
                'tag_str': tag_str}
        url = 'http://www.lawcdyc.com/index.php'
    ...
    def post_image(self):
        """上传略缩图 
        请求 URL: http://www.lawcdyc.com/index.php?m=mod&c=thumb&a=thumb_upload
        表单：
        logo: (二进制)
        typename: article
        item_id: 20203
    ...
    def list_remote_images(self, cat_id, page):
        """查询空间中的略缩图
        请求 URL: http://www.lawcdyc.com/index.php?m=mod&c=thumb&a=picture_list
        ...
        """
        url = "http://www.lawcdyc.com/index.php?m=mod&c=thumb&a=picture_list"
    ...
    def post_remote_image(self, file_path):
        """上传空间中的略缩图
        请求 URL: http://www.lawcdyc.com/index.php?m=mod&c=thumb&a=picture_2_thumb
        ...
        url = 'http://www.lawcdyc.com/index.php?m=mod&c=thumb&a=picture_2_thumb'
        rep = requests.post(url, data=form, headers=self.headers)
        return rep.text

    def check_image(self):
        """查询略缩图
        请求 URL: http://www.lawcdyc.com/index.php?m=mod&c=thumb&a=thumb_list
    ...
    def post_article(self, tag_content):
        """发布文章内容
        请求URL: http://www.lawcdyc.com/index.php?m=site&c=content&a=article_form
        ...
        info[status]: 1
        status: 1
        http_referer: http://www.lawcdyc.com/index.php?m=site&c=content&a=article_index
