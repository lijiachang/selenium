...
if len(sys.argv) != 3:
    print("||%s||FM-PLUGIN-EXECUTE-FAILED||error:need 2 paramters,dip and ping count" % sys.argv[0])
    sys.exit(0)
...
def pinger(ip, count):
    ...
    if ret == 0:
        result = curip + '||' + ip  + '||APM-LINUX-Ping-01-05||0'

        print(result)
    else:
        result = curip + '||' + ip + '||APM-LINUX-Ping-01-05||1'
