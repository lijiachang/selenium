...
try:
    ...
    print(
        "||%s||FM-PLUGIN-EXECUTE-FAILED||Please usage:%s ip port user password servername" % (sys.argv[0], sys.argv[0]))
    sys.exit(0)
...
if password == 'None':
    ...
    cmd = 'mysql -h ' + hostip + ' -P ' + port + ' -u ' + username + ' -p' + password + ' ' + database + ' -e "show status"'
...
######## monitor keys list#####################
keyl = ['Innodb_buffer_pool_pages_dirty']
...
for i in L:
    if i['Variable_name'] in keyl:
        ...
        res = hostip + "-" + port + "-" + database + '||||APM-' + i['Variable_name'] + '||' + i['Value']
        print(res)
