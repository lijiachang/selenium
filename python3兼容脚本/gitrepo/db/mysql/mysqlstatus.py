...
def output(sql):
    ...
    if b"Unknown database" in stderr:
        print(separator.join([mpoint, ckbp, "PM-mysql-status", "1"]))
    elif b"ERROR 1045 (28000)" in stderr:
        print(separator.join([mpoint, ckbp, "PM-mysql-status", "2"]))
    elif b"ERROR 2003 (HY000)" in stderr or b"ERROR 2005" in stderr:
        print(separator.join([mpoint, ckbp, "PM-mysql-status", "3"]))
    elif b"ERROR" in stderr or b"command not" in stderr:
