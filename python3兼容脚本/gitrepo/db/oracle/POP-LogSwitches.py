...
if __name__ == "__main__":
    sqlComm = """select *
  from (select to_char(trunc(first_time, 'HH'), 'yyyy-mm-dd hh24') time,
               count(*) COUNT
          from v\$loghist
         group by to_char(trunc(first_time, 'HH'), 'yyyy-mm-dd hh24')
         order by time desc)
 where to_date(time, 'yyyy-mm-dd hh24') > sysdate -1 and COUNT>20;"""  # ����$ ��Ҫת��
    ...
    for row in result:
        ...
        results.append(separator.join([mpoint, "PM-ORACLE-PERFORMANCE-LogSwitches", ckbp,row.get("COUNT")]))# TIME+COUNT
