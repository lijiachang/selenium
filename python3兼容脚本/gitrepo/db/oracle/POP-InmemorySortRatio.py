...
if __name__ == "__main__":
    sqlComm = """select round((1 - disk.value / (disk.value + memory.value)) * 100, 2) "In-Memory Sort"
  from v\$sysstat disk, v\$sysstat memory
 where disk.name = 'sorts (disk)'
   and memory.name = 'sorts (memory)';"""  # ����$ ��Ҫת��
    ...
    results = list()
    results.append(separator.join([mpoint, ckbp, "PM-ORACLE-PERFORMANCE-InmemorySortRatio", result[0].get("In-Memory Sort")]))  
