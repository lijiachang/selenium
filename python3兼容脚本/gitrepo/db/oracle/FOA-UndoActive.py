...
def QueryBySqlPlus(sqlCommand):
    ...
    if not list_1:
        ...
        print("%s||||FM-ORACLE-AVAIABILITY-UndoActive||0" % mpoint)  # SQL�޷�������������
        sys.exit(0)
...
if __name__ == "__main__":
 ...
 where tablespace_name =
       (select value from v\$parameter where name = 'undo_tablespace') and status='ACTIVE'
 group by status;"""  # ����$ ��Ҫת��
    ...
    for row in result:
        ...
        results.append(separator.join([mpoint, "FM-ORACLE-AVAIABILITY-UndoActive", ckbp,row.get("SIZE_M")]))# TIME+COUNT
